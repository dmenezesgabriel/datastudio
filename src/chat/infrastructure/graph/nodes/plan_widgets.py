...
_SYSTEM_PROMPT = (
    ...
    "  Also choose 'text' when the question asks for a metric the schema cannot support — e.g. "
    "a profit, margin, or cost figure when no cost column exists. In `text_answer`, say plainly "
    "the metric cannot be computed from the available data, name the closest metric you CAN "
    "compute, and never invent a proxy column or route it to SQL: a fabricated number is worse "
    "than an honest 'not available'.\n"
